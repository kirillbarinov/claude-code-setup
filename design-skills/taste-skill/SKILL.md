...
### 0.0 Ask before you infer — inference is the fallback, not the first move

Inference is what you do when asking is impossible. If you have a live channel to the person
(`AskUserQuestion`) and the style axis is still open — a new site, a landing, a redesign — **ask first**,
one round, at most four questions: where the style comes from (a reference they like / invent it from
scratch / show me 2–3 rough directions), what must NOT be there, tone as a choice between two
distinguishable options, and who looks at it on what device. "Decide for me" is a complete and final
answer: after it, no more questions for the rest of the task.

If they name a reference, the follow-up matters more than the first round: **what exactly to take from
it and what to leave** — layout / typography / color / density / motion / copy tone. "Make it like
Stripe" without that means "make another Stripe", and what they actually liked there is usually one
single thing.

Do not ask inside a project whose style is already fixed (a `DESIGN.md` exists, the edit is scoped to
one section), when the brief already describes the style, or on a repeat pass of the same work. Never
ask about radii, shadows, font names or type scale — that is your job, and asking hands it back to them.

Full protocol, with the "decide for me" mode and its reporting rules: §1в of
`~/.claude/agents/design-director.md`. When there is no channel to a human, go to "decide for me" mode
and say so in the report — silently deciding for someone you could have asked is the failure this
...
1. **Generate through OpenRouter.** Local rule, overrides anything else in this file: every image and video is produced by the `openrouter-images` / `openrouter-video` skills. No other generator may be called — not `generate_image`, not an MCP image tool, not an IDE-integrated one, not OpenAI/Midjourney/DALL-E/Gemini/Imagen/fal.ai/Replicate/Higgsfield. Create section-specific assets: hero photography, product shots, texture backgrounds, mood images, each at the right aspect ratio for its section. Do not skip this step because hand-rolled CSS feels faster.
2. **No stock-photo fallback.** `picsum.photos`, Unsplash, Pexels and any other placeholder host are forbidden here, including "temporarily". A page shipped with a placeholder counts as unfinished work, not as a draft.
3. **If generation fails: stop and say so.** If neither is possible, do NOT fill the page with hand-rolled SVG illustrations or div-based "fake screenshots." Instead, leave clearly-labeled placeholder slots (`<!-- TODO: hero product photo, 1600x1200 -->`) and at the end of the response say: *"This page needs real images at: \[list of placements\]. Please generate or provide them."*
...
* **NO stock or placeholder image hosts at all** — no Unsplash, no Pexels, no `picsum.photos`. Every image is either generated via `openrouter-images` or a real project asset (see Section 4.8).
...
- [ ] **Real images used** (generated via `openrouter-images`, or real project assets - never Picsum/Unsplash/Pexels) - NO div-based fake screenshots, NO hand-rolled decorative SVGs, NO pure-text minimalism?
