...
- **Image Context & Style:** Generate every image with the `openrouter-images` skill (local rule: no `picsum.photos`, Unsplash, Pexels or any other placeholder host, not even temporarily). Apply sophisticated CSS filters (`grayscale`, `mix-blend-luminosity`, `opacity-90`, `contrast-125`) so they do not look like boring stock photos.
