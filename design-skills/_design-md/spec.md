...
> Локальное дополнение (не часть генератора выше): формат и диапазоны блока `motion:` во фронтматтере — `_design-md/motion.md` §3; правила контракта для существующего проекта (brownfield) — `_design-md/brownfield.md`.
